...
public class SystemApp {
	static MP3Player myPlayer = new Jukebox();// Initialize MP3Player object
	static Jukebox myJukebox =  (Jukebox)myPlayer;// Object casting
	static final String options[] = { "Select from Full List", "Select Tune by Artist", "Select Tune by Genre",
			"Add New Tune", "Display the Top 10", "Switch Off", "Switch On","Purchase Credits","Set Cost Per Credit","Exit" };// Options for creating the menu
	...
	private static void processChoice(int choice) {
		 ...
		 * All options - Select from full list,Select Tune by Artist,Select Tune by Genre,Add New Tune,Display the Top 10,Switch Off,Switch On,Purchase Credits,Set Cost Per Credit
		 */
		if(choice == 1)	{	
            ...
            int choice1 = input.nextInt(); // User input for genre
            System.out.println();
            /*
			...
			if (isOn == true) {
			System.out.println("Enter the denomination of the coin"); // User input denomination of coin
			int coin = input.nextInt();
			input.nextLine();
			purchaseCredits(coin);
			}
			else {
				System.out.println("MP3 Player is off");
			}
			...
			if (isOn == true) {
			System.out.println("Enter the cost per credit to be set"); // User input cost per credit
	...
	/**
	 * This function allows the user to purchase credits using coins with the following denominations - 10p,20p,50p,1 pound(100p),2 pounds(200p)
	 * @param coin - the denomination of the coin
	 */
	private static void purchaseCredits(int coin) {
		if (myJukebox.getCostPerCredits() != 0) { // Checks if cost per credits is not 0
        if (coin == 10 || coin == 20 || coin == 50 || coin == 100 || coin == 200) { // Checks if the coin is of the right denomination
        	myJukebox.insertCoin(coin);
        	System.out.println("Money converted to credits: successful");
        

    }
        else {
        	System.out.println("Incorrect denomination. Please try again!"); // Validation message when coin is of incorrect denomination
        }
	}
		else {
			System.out.println("The price of costPerCredits is 0. Please set it to a positive value before trying to purchase credits"); // Validation message when cost per credit is 0
		}
	}
	/**
	 * This function allows the user to change the cost per credit
	 * @param cost - the cost per credit
	 */
	private static void callSetterCost(int cost) { 
		myJukebox.setCostPerCredits(cost);
		System.out.println("Cost Per Credits set up succcessfully!");
	}
